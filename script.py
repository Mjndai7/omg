...
def resolve_url(url, signature):
    ...
    try:
        # Follow redirects instead of using resolve endpoint
        ...
        logging.warning(f"Resolution failed for {url}: {e}, using original")
        return url
...
def generate_m3u(channels_json, signature=None, filename="/app/generated_playlist.m3u"):
    ...
    print(f"M3U8 file generated successfully: {filename} with {playable_count} streams")
